...
* `name` - (Optional, String, ForceNew) Specifies the name of the namespace. Must be unique. This parameter can
  ...
  Exactly one of `name` or `generate_name` must be provided.

* `generate_name` - (Optional, String, ForceNew) Specifies A prefix used by the server to generate a unique name.
  ...
  Changing this will create a new namespace resource. Exactly one of `name` or `generate_name` must be provided.
